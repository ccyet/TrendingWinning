...
def test_legacy_backtest_trailing_take_profit_ignores_zero_liquidity_extreme() -> None:
    scanned = pd.DataFrame(
        {
            "date": pd.to_datetime(
                [
            ...
            "stock_code": ["000001.SZ"] * 4,
            "open": [10.0, 10.2, 11.9, 11.7],
            "high": [10.1, 10.3, 12.0, 11.8],
            "low": [9.9, 10.1, 11.8, 11.5],
            "close": [10.0, 10.2, 11.9, 11.6],
            "volume": [1000.0, 1200.0, 0.0, 1300.0],
            "amount": [10000.0, 12240.0, 0.0, 15080.0],
            "breakout_trigger": [True, False, False, False],
            "trigger_price": [10.0, pd.NA, pd.NA, pd.NA],
        }
    )
...
def test_legacy_backtest_resolves_stop_and_trailing_hit_in_same_bar_conservatively() -> None:
    scanned = pd.DataFrame(
        {
            "date": pd.to_datetime(
                [
            ...
            "high": [10.1, 10.3, 11.0, 10.9],
            "low": [9.9, 10.1, 10.7, 9.4],
            "close": [10.0, 10.2, 10.9, 10.2],
            "volume": [1000.0, 1200.0, 1300.0, 1400.0],
            "amount": [10000.0, 12240.0, 14170.0, 14280.0],
            "breakout_trigger": [True, False, False, False],
            "trigger_price": [10.0, pd.NA, pd.NA, pd.NA],
        }
    )
