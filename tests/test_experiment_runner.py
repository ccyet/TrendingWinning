...
_PARAMETER_DECISION_METRIC_PAIRS = (
    ...
    ("avg_accepted_actual_risk_pct", "avg_accepted_actual_risk_pct"),
    ("avg_accepted_actual_chase_pct", "avg_accepted_actual_chase_pct"),
    ("avg_accepted_actual_reward_to_risk", "avg_accepted_actual_reward_to_risk"),
    ("avg_executed_actual_risk_pct", "avg_executed_actual_risk_pct"),
    ("avg_executed_actual_chase_pct", "avg_executed_actual_chase_pct"),
    ("avg_executed_actual_reward_to_risk", "avg_executed_actual_reward_to_risk"),
...
_PARAMETER_EFFICIENCY_METRIC_PAIRS = (
...
def _assert_parameter_summary_efficiency_metrics(
    saved_parameter_summary: pd.DataFrame,
    saved_sweep: pd.DataFrame,
    *,
    parameter: str,
...
def test_portfolio_experiment_saves_reproducible_config_and_outputs(tmp_path: Path) -> None:
    ...
    assert (output_dir / "data_gap_episodes.csv").exists()
...
def test_portfolio_parameter_sweep_reuses_loaded_data_and_saves_ranked_table(tmp_path: Path, monkeypatch) -> None:
    ...
    assert saved_summary["case_setup_row_count"] == len(saved_case_setup)
    assert saved_summary["case_setup_trade_count"] == pytest.approx(saved_case_setup["trade_count"].sum())
    assert saved_summary["case_setup_zero_trade_row_count"] == int(saved_case_setup["trade_count"].eq(0).sum())
    assert saved_summary["case_symbol_row_count"] == len(saved_case_symbol)
    assert saved_summary["case_symbol_trade_count"] == pytest.approx(saved_case_symbol["trade_count"].sum())
    assert saved_summary["case_symbol_zero_trade_row_count"] == int(saved_case_symbol["trade_count"].eq(0).sum())
