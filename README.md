...
单策略和组合策略参数区会在运行前展示“策略执行空间”，把当前样本、识别形态、适用空间、信号条件、触发成交、开仓过滤、退出条件、仓位规则、或然分支和复盘输出翻译成一张表，方便先确认哪些模块会生效、哪些参数只属于组合层、哪些订单会进入过滤或撮合。策略空间不是一个单项参数，而是样本、形态、过滤、执行、退出、统计六类边界。
信号条件会明确写出多头/空头信号 K、入场触发价、结构止损和成交确认方式；触发成交条件写明为多头 high >= 入场触发价、空头 low <= 入场触发价；或然分支会把无信号、观察信号、有效信号、有效未触发、触发成交、触发后拒单、持仓冲突、容量/资金拒单和退出完成拆开。
...
`experiment_diagnostics.csv` 把数据覆盖、交易样本、订单接受率、策略过滤、回撤压力、收益质量、胜率边际、正期望概率、退出结构、月度稳定性、路径风险和资金暴露汇总为 `通过 / 关注 / 失败`，用于优先定位该先修数据、调信号、调风控、调胜率边际、调正期望概率、调退出还是调仓位。
