...
回测结果页会展示“策略K线运行区间”，按股票切换完整样本 K 线，横轴按连续 K 序号压缩并支持缩放；图上标注开多、开空、平仓、止损和回撤止盈，悬停只显示价格、具体开仓/平仓时间和开仓/平仓原因，方便检查信号和风控是否落在正确 K 线上。
...
单策略和组合策略不使用旧突破的固定百分比止盈止损控件：止损价由各形态识别模块输出，目标价由信号 K 风险距离和 `risk_reward` 计算，回撤止盈再作为成交后的动态退出模块叠加。
