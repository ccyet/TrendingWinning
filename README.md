...
回测结果页会展示“策略K线运行区间”，按股票切换完整样本 K 线，横轴按连续 K 序号压缩，横轴和价格轴都支持缩放；长样本不会受 5000 行默认渲染限制，并使用 SVG 矢量渲染保持放大后的清晰度。图上标注开多、开空、平仓、止损和回撤止盈，悬停只显示价格、具体开仓/平仓时间和开仓/平仓原因，方便检查信号和风控是否落在正确 K 线上。
...
单策略和组合策略不使用旧突破的固定百分比止盈止损控件：止损价由各形态识别模块输出，目标价由信号 K 风险距离和 `risk_reward` 计算；Web 页面可用“启用回撤止盈”开关，把成交后的动态回撤止盈显式打开或关闭。
...
单策略和组合策略的固定目标价由信号 K 止损距离乘以 `risk_reward` 得出，Web 页面不会再展示旧突破专用的固定百分比止盈止损输入，避免参数改了但现代策略不生效。
...
固定参数可直接用 `--trailing-take-profit-activation-pct / --trailing-take-profit-drawdown-pct`，需要遍历时也可用通用 `--grid trailing_take_profit_activation_pct=0,0.03 --grid trailing_take_profit_drawdown_pct=0,0.01`。
