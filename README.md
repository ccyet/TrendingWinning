...
单策略和组合策略参数区会在运行前展示“策略执行空间”，把当前样本、识别形态、适用空间、信号条件、触发成交、开仓过滤、退出条件、仓位规则、或然分支和复盘输出翻译成一张表，方便先确认哪些模块会生效、哪些参数只属于组合层、哪些订单会进入过滤或撮合。
信号条件会明确写出多头/空头信号 K、入场触发价、结构止损和成交确认方式；或然分支会把有效信号、有效但未触发、过滤拒单、撮合拒单、持仓冲突、容量/资金拒单和退出归因拆开。
