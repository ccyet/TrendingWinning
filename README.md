...
CLI 保存回测或参数遍历时会打印 `artifact_manifest.csv saved:`，Windows 侧终端测试后先按这行打开产物索引，再决定查看成交、净值回撤、参数遍历或数据质量文件。
已有输出目录也可以直接运行 `python -m trending_winning.cli show-artifacts --output-dir runs/single-trend-001`，在终端按优先级查看产物索引表和每个文件的本机路径。
目录很长时可加 `--max-priority 1` 只看最该先读的文件，也可加 `--category 数据质量` 只看某一类产物。
