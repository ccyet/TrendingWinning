...
TDX 需要本机通达信已启动并登录，并把 `TDX_TQCENTER_PATH` 指向 `PYPlugins/user`，也可以在页面侧边栏填写。`60m` 会按 TDX 接口要求映射为 `1h` 请求；分钟线是否能返回数据取决于客户端本地是否已下载对应周期数据。
...
python -m trending_winning.cli tdx-doctor \
  ...
  --tdx-path /path/to/PYPlugins/user
...
`tdx-doctor` 不写本地文件，只诊断 TDX 通道：逐周期输出 `ok / no_data / request_error / init_error`、TDX 实际 period、返回行数、样本起止时间和错误信息。先用它确认通达信已启动登录、`PYPlugins/user` 路径有效、`5m/15m/30m/60m/1d` 都能返回样本，再执行补数和回测。
...
docker run --rm -p 8520:8501 \
  ...
  -e TDX_TQCENTER_PATH=/path/to/PYPlugins/user \
  trending-winning
```

容器内默认可把页面数据目录改成 `/data/market/daily`。
