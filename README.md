...
`experiment_diagnostics.csv` 把数据覆盖、交易样本、订单接受率、策略过滤、回撤压力、收益质量、退出结构、月度稳定性、路径风险和资金暴露汇总为 `通过 / 关注 / 失败`，用于优先定位该先修数据、调信号、调风控、调退出还是调仓位。
