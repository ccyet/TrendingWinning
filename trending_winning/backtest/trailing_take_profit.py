...
def trailing_take_profit_enabled(activation_pct: float, drawdown_pct: float, ma_period: int = 0) -> bool:
    """判断回撤止盈是否启用；比例回撤和均线回撤可独立于启动浮盈生效。"""
    return bool(drawdown_pct > 0 or ma_period >= 2)
...
def trailing_take_profit_masks(
    ...
    *,
    ...
    多头可先等上一根完成 K 的最高价相对实际入场价达到启动浮盈，再用
    ...
    if side == "long":
        ...
        armed = _long_activation_mask(previous_peak, entry_price, activation_pct)
    ...
    if side == "short":
        ...
        armed = _short_activation_mask(previous_trough, entry_price, activation_pct)
        active = np.isfinite(prices)
        reported_prices = _reported_prices(prices, ratio_prices, ma_prices)
        return TrailingTakeProfitMasks(
            gap=liquid & armed & active & (opens >= prices),
...
def _long_activation_mask(previous_peak: np.ndarray, entry_price: float, activation_pct: float) -> np.ndarray:
    if activation_pct <= 0:
        return _immediate_activation_mask(len(previous_peak))
    return previous_peak >= entry_price * (1.0 + activation_pct)


def _short_activation_mask(previous_trough: np.ndarray, entry_price: float, activation_pct: float) -> np.ndarray:
    if activation_pct <= 0:
        return _immediate_activation_mask(len(previous_trough))
    return previous_trough <= entry_price * (1.0 - activation_pct)


def _immediate_activation_mask(length: int) -> np.ndarray:
    armed = np.full(length, True)
    if length:
        armed[0] = False
    return armed
